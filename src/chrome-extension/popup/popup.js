  ...
  chrome.storage.local.get("hiddenTabs", (result) => {
    ...
    if (hiddenTabs.length === 0) {
      ...
      hiddenTabs.forEach(tab => {
        ...
        actionCell.style.minWidth = "150px";
...
document.getElementById("hideAllTabsBtn").addEventListener("click", () => {
  chrome.storage.local.get("customUrl", (result) => {
    ...
    chrome.runtime.sendMessage({ action: "hideAllTabs", customUrl: customUrl }, () => {
      loadHiddenTabs();
    });
  });
});
...
document.getElementById("unhideAllTabsBtn").addEventListener("click", () => {
  chrome.runtime.sendMessage({ action: "unhideAllTabs" }, () => {
    var hiddenTabsList = document.getElementById("hiddenTabsList");
    hiddenTabsList.innerHTML = '';
    loadHiddenTabs();
  });
  ...
  chrome.storage.local.get("hiddenTabs", (result) => {
    ...
    if (tabIndex !== -1) {
      ...
      chrome.storage.local.set({ hiddenTabs }, () => {
        chrome.runtime.sendMessage({ action: "unhideTab", tabId: tab.id }, () => {
          loadHiddenTabs();
        });
      });
    }
